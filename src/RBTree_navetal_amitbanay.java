...
public class RBTree_navetal_amitbanay
{
	 ...
	 * public class RBNode
	 * <p>
	 * A class with well named functions and fields that explain themselves
	 * (*with exceptions).
	 */
	public static class RBNode
	{
		 ...
		 * "dummy" key that represents right/left-ness, null value, and no
		 * children, 0 size. Perfect for dummy nodes (usually representing null
		 * "leaves")!
		 *
		 * @param parent
		 */
			...
			if (currentNode.getParent().getNodeColor() == NodeColor.RED)
			{
				...
				} else
				{
					...
					// uncle
					{
						...
						int cs = C.getSize();
						B.setSize(cs);
						C.setSize(C.getSize() - (1 + B.left.getSize())); // No
																			// need
																			// to
																			// check
																			// if
					...
					} else // Cases 2+3 - mirrored
					{
						...
						int cs = C.getSize();
						B.setSize(cs);
						C.setSize(C.getSize() - (1 + B.right.getSize())); // No
																			// need
																			// to
																			// check
																			// if
	 ...
	 * must remain valid (keep its invariants). returns the number of color
	 * switches, or 0 if no color switches happened. returns -1 if an item with
	 * key k was not found in the tree.
	 */
	public int delete(int k)
	{
		...
		// The color that we have removed from the tree and might cause
		// problems.
	 ...
	 * array if the tree is empty. The function picks the keys with an inorder
	 * recursion and thus runs in O(n) time.
	 */
	public int[] keysToArray()
	{
	 ...
	 * respective keys, or an empty array if the tree is empty. The function
	 * uses inorder recursion to pick the nodes' values according to the keys
	 * order thus runs in O(n) time.
	 */
	public String[] valuesToArray()
	{
	...
	/**
	 * public int size() The function runs in O(1) time Dahhh.
	 * <p>
	 ...
	 * not his child anymore. The function runs in O(1) time.
	 *
	 * @param oldNode
	 ...
	 * node-size = 1 + left-child-size + right-child-size Unless you do
	 * something crazy make sure the children's size is updated before calling
	 * updateSize! The function runs in O(1) time.
	 *
	 * @param node
	 *            The node to update it's size.
	 */
	...
	/**
	 * Updates the size of the given node and all it's ancestors. The function
	 * runs in O(h)=O(lg n) time.
	 *
	 * @param node
	 *            The node to update it's size.
	 */
	...
	/**
	 * returns the node with the key k if it exists, else returns null. The
	 * function uses a binary search to find the node thus it runs in O(log(n)).
	 *
	 * @param k
	 ...
	 * if it's key is DUMMY_RIGHT_KEY. The function runs in O(1) time.
	 *
	 * @param node
	 *            the node that is checked, if the node is null a "dummy" node
	 *            should be passed.
	...
	/**
	 * Makes a right rotate with the given node as the "pivot" and updates the
	 * sizes accordingly. The function runs in O(1) time.
	 *
	 * @param node
	...
	/**
	 * Makes a left rotate with the given node as the "pivot" and updates the
	 * sizes accordingly. The function runs in O(1) time.
	 *
	 * @param node
	...
	/**
	 * Returns the successor of the given node in it's sub tree The function
	 * uses getMinimum and thus runs in O(lg n) time.
	 *
	 * @param node
	 *            A node to look for his successor, may be null.
	 * @return The successor of the node in it's sub tree, null if node is null
	 *         or the node itself if it is'nt null but there is no successor.
	 */
	...
	/**
	 * Get the node with the minimal key in the subtree of the given node. The
	 * function travels down the left path of node's subtree and thus runs in
	 * O(h) = O(lg n) time.
	 *
	 * @param node
