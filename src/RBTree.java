...
public class RBTree {
    ...
    public static class RBNode {
         ...
         * "dummy" key that represents right/left, null value, and no children. Perfect for dummy nodes!
         *
         * @param parent
         */
        public RBNode(RBNode parent, boolean isRight) {
            this.setKey(isRight ? DUMMY_RIGHT_KEY : DUMMY_LEFT_KEY);
            this.setValue(null);
            this.setParent(parent);
            this.setLeft(null);
            this.setRight(null);
    ...
    public int delete(int k) {
        ...
        if (deleted.getLeft() == null) {
            ...
            if (fixNode == null) {
                fixNode = new RBNode(deleted.getParent(), isDeletedRightChild); // Creating a dummy
                // node representing
                // the null "node".
            }
            ...
            if (successor.getParent() != deleted) {
                ...
                if (fixNode == null) {
                    fixNode = new RBNode(successor.getParent(), isSuccessorRightChild); // Creating a
                    // dummy
                    // node
                    // representing
                    // the null
    ...
    /**
     * returns weather the RBNode node is a right child or not. If the given node is "dummy"
     * (not recognized by his parent) then the procedure checks if it's key is DUMMY_RIGHT_KEY.
     *
     * @param node the node that is checked, if the node is null a "dummy" node should be passed.
     *
     * @return true if node is a right child of node.parent, false if it is a left child.
     */
        ...
        while (fixNode.parent != sentinel && fixNode.color == NodeColor.BLACK) {
            if (isRightChild(fixNode)) {
                ...
                if ((brother.getLeft() == null || brother.getLeft().getNodeColor() == NodeColor.BLACK)
                    ...
                    if (brother.getLeft() == null || brother.getLeft().getNodeColor() == NodeColor.BLACK) {
                        colorSwitches++;
                        brother.getRight().setNodeColor(NodeColor.BLACK); // A red right node exists because only left is black
                        brother.setNodeColor(NodeColor.RED);
                        leftRotate(brother);
                ...
                if ((brother.getLeft() == null || brother.getLeft().getNodeColor() == NodeColor.BLACK)
                    ...
                    if (brother.getRight() == null || brother.getRight().getNodeColor() == NodeColor.BLACK) {
                        colorSwitches++;
                        brother.getLeft().setNodeColor(NodeColor.BLACK); // A red left node exists because only right is black.
